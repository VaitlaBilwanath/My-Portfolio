...
class AppModel {
  ...
  onTouchUp() {
    ...
    if (!this.hasMoved && this.onItemClick && this.medias && this.medias[0]) {
      const width = this.medias[0].width;
      const activeIndex = Math.round(this.scroll.target / width);
      const originalLength = this.originalItems.length;
      if (originalLength > 0) {
        const resolvedIndex = ((activeIndex % originalLength) + originalLength) % originalLength;
        const clickedItem = this.originalItems[resolvedIndex];
  ...
  onKeyDown(e) {
    switch (e.key) {
      case 'ArrowRight':
        e.preventDefault();
        this.scroll.target += this.scrollSpeed * 6;
        this.onCheckDebounce();
        break;
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      app = new AppModel(containerRef.current, {
        items,
        bend,
        textColor,
        borderRadius,
